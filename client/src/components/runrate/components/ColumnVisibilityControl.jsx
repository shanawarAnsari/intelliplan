...
const ColumnVisibilityControl = ({
  ...
  const handleToggle = (columnId) => {
    if (ESSENTIAL_COLUMNS.includes(columnId)) return;

    const newVisible = visibleColumns.includes(columnId)
      ? visibleColumns.filter((id) => id !== columnId)
      : [...visibleColumns, columnId];

    onVisibilityChange([...new Set([...ESSENTIAL_COLUMNS, ...newVisible])]);
  };

  const handleSelectAll = () => {
    onVisibilityChange([
      ...new Set([...ESSENTIAL_COLUMNS, ...columns.map((c) => c.id)]),
    ]);
  };
            ...
            <Box key={group.name}>
              ...
              >
                ...
                >
                  {group.name}
                  <Chip
                    label={`${group.columns.filter((c) => visibleColumns.includes(c.id))
                        .length
              ...
              <List dense sx={{ py: 0, backgroundColor: "#7f1d1d" }}>
                {group.columns.map((column) => {
                  ...
                  const isEssential = ESSENTIAL_COLUMNS.includes(column.id);

                  return (
                    <ListItem key={column.id} disablePadding>
                      <ListItemButton
                        onClick={() => !isEssential && handleToggle(column.id)}
